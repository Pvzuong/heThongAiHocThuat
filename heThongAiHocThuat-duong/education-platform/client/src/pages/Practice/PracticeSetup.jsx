...
const PracticeSetup = () => {
  ...
  return (
    <div className="ps-page">
      ...
      {/* ── Practice Modes ── */}
      <div className="ps-modes-grid">
        {[
          { icon: '🎯', label: 'Daily Challenge', desc: '10 câu · +50 XP', link: '/practice', color: 'var(--primary)',   bg: 'var(--primary-light)' },
          { icon: '🤖', label: 'AI Practice',     desc: 'AI sinh câu hỏi cá nhân', link: '/practice/ai-generator', color: 'var(--secondary)', bg: 'var(--secondary-light)' },
          { icon: '🏆', label: 'Ranked',          desc: 'Bronze → Diamond',   link: '/ranking',    color: 'var(--xp-color)', bg: 'var(--warning-light)' },
          { icon: '⚡', label: 'Speed Run',       desc: '60 giây thi đua',    link: '/practice',   color: 'var(--accent-dark)',   bg: 'var(--accent-light)' },
        ].map(m => (
          <Link key={m.label} to={m.link} className="ps-mode-card" style={{ '--mc': m.color, '--mb': m.bg }}>
            <span className="ps-mode-card-icon" style={{ background: m.bg, color: m.color }}>{m.icon}</span>
            <div className="ps-mode-card-label">{m.label}</div>
            <div className="ps-mode-card-desc">{m.desc}</div>
          </Link>
        ))}
      </div>
