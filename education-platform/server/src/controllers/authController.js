...
// ─── Helper: kiểm tra rate limit OTP ────────────────────────
const checkOTPRateLimit = async (email) => {
  const row = await pool.query(
    `SELECT request_count, window_start FROM otp_rate_limits WHERE email = $1`, [email]
  );
  if (row.rows.length > 0) {
    ...
    if (ageMin >= RATE_WINDOW_MINUTES) {
      await pool.query(
        `UPDATE otp_rate_limits SET request_count = 1, window_start = NOW(), last_request_at = NOW() WHERE email = $1`, [email]
      );
    } else {
      await pool.query(
        `UPDATE otp_rate_limits SET request_count = request_count + 1, last_request_at = NOW() WHERE email = $1`, [email]
      );
    }
  } else {
    await pool.query(`INSERT INTO otp_rate_limits (email) VALUES ($1)`, [email]);
  }
