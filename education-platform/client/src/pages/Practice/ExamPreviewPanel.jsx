...
export default function ExamPreviewPanel({ result, form, setResult, onRegenerate }) {
  ...
  const handleClone = async () => {
    try {
      const res = await geminiApi.cloneCollection(collectionId);
  ...
  const handleSaveEdit = async (q, data) => {
    setSavingId(q.id);
    try {
      const res = await geminiApi.updateQuestion(collectionId, q.id, data);
      setResult(r => ({
        ...r,
  ...
  const handleDelete = async (q) => {
    ...
    setDeletingId(q.id);
    try {
      await geminiApi.deleteQuestion(collectionId, q.id);
  ...
  const handleRegenerate = async (q) => {
    ...
    try {
      const res = await geminiApi.regenerateQuestion(collectionId, q.id);
      setResult(r => ({
        ...r,
        questions: r.questions.map(item => item.id === q.id ? res.data.question : item),
      }));
